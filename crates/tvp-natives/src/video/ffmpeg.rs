//! FFmpeg-backed movie decoding (compiled only with the `ffmpeg` feature).
//!
//! This submodule owns everything that links against the system FFmpeg
//! libraries (`libavformat` + `libavcodec` + `libswscale` +
//! `libswresample`). The public API it exposes is re-exported by the parent
//! [`video`](super) module and does not change with the configuration; the
//! `unsupported` submodule provides the identical API when the feature is
//! off.

use super::{DecodedAudioPcm, MovieMetadata, RgbaFrame, init};
